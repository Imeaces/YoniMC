import { EventTypes, Logger } from "yoni-mcscripts-lib";
import { ObjectUtils } from "yoni-mcscripts-lib";
const { getKeys } = ObjectUtils;
const logger = new Logger("TEST");
EventTypes.get("minecraft:beforeItemUseOn").subscribe(function sub0(event) {
    logger.info("start sub0 {}", event.cancel);
    logger.info("end sub0 {}", event.cancel);
});
EventTypes.get("minecraft:beforeItemUseOn").subscribe(function sub1(event) {
    logger.info("start sub1 {}", event.cancel);
    event.cancel = true;
    logger.info("end sub1 {}", event.cancel);
});
EventTypes.get("minecraft:beforeItemUseOn").subscribe(function sub2(event) {
