import { ChatCommand, EventListener, Command, EntityBase, Location, Minecraft, Logger } from "yoni-mcscripts-lib";
import { Dimension } from "yoni-mcscripts-lib";
import { FakePlayerManager } from "./../fakePlayer/FakePlayer.js";
//////////////////////
//     config       ///
//////////////////////
...
async function getDiggerPlayer() {
    let digger;
    try {
        digger = FakePlayerManager.getFakePlayer("§§DiggerHelper");
    }
    catch {
        digger = await FakePlayerManager.createFakePlayer(0, { x: 0, y: 0, z: 0 }, "§§DiggerHelper");
        await digger.fetchCommand("function yonimc/lock_inventory");
    }
    return digger;
}
let holdingPros = new Promise((re) => re());
...
async function makeDiggerPlayer(digger, player) {
    ...
    try {
        let comp = digger.getComponent("scale");
        comp.value = 0.0001;
    }
    catch { }
...
async function digBlock(digger, location, player, gamemode) {
...
//  chain method 1  ///
//////////////////////
function getOneDimensionDirection(v) {
    if (v > 0) {
        return 1;
    }
    else if (v < 0) {
...
function ChainDestroy1_addOffset(blockStack, offset, depth) {
    let offsetInfo = { offset, depth, canNextOffsets: [] };
    let [x, y, z] = offset;
...
async function startChainDestroy1(center, player, blockType, radius = Config.maxRadius) {
    let blockStack = [];
    for (let i = -1; i <= 1; i++) {
        for (let j = -1; j <= 1; j++) {
            for (let k = -1; k <= 1; k++) {
    ...
    await (async (fakePlayerReq) => {
        ...
        player.getInventory().setItem(player.selectedSlot, digger.getInventory().getItem(digger.selectedSlot));
        digger.teleport(diggerPlayerIdleLocation);
    })()
        .catch(logger.error)
        .finally(fakePlayerReq.release);
}
async function chainDestroy1(digger, player, center, blockStack, blockType, radius) {
    let gamemode = player.gamemode;
    while (blockStack.length > 0) {
        let offsetInfo = blockStack.shift();
        ...
        for (let ofinfo of offsetInfo.canNextOffsets) {
            // @ts-ignore
            let offset = Array.from(offsetInfo.offset);
            offset[0] += ofinfo[0];
            offset[1] += ofinfo[1];
            offset[2] += ofinfo[2];
...
//  chain method 2  ///
//////////////////////
function getLocStr(v) {
    return `${v.x},${v.y},${v.z}`;
}
function getStrLoc(v) {
    return new Location(v.split(",").map(Number));
}
function getSurroundingLocs(v) {
    let rt = [];
...
async function startChainDestroy2(center, player, blockType, chainCount = Config.maxChainCount) {
    logger.debug("startChainDestroy2:: center: {}", center.toString());
    //存储方块坐标字符串
    let detectBlockStack = [];
    let detectedLocs = new Set();
    let dimension = center.dimension;
    detectBlockStack.push(...getSurroundingLocs(center).map(v => getLocStr(v)));
    let BlockTypeGroup = getBlockTypeGroup(blockType.id);
    let _startTime = Date.now();
    let fakePlayerReq = await holdDiggerPlayer();
    if (Date.now() - _startTime > 5000) {
        fakePlayerReq.release();
...
const group_blocktype_definitions = [
