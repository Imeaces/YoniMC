import { EventListener, Minecraft, YoniScheduler as Scheduler, world, EntityBase } from "yoni-mcscripts-lib";
const fireRecoverTime = 55;
const fireCooldownTimer = new Map();
...
EventListener.register("system:afterEvents.scriptEventReceive", (event) => {
...
function despawn(player) {
    const comp = player.getComponent("minecraft:equipment_inventory");
    console.log("despawn");
...
Scheduler.runCycleTickTask(function getHungerOnGliding() {
    world.getAllPlayers().filter(updateWingingPlayer)
        ...
        .forEach(player => {
        appendEffect(player, Minecraft.MinecraftEffectTypes.hunger, fireRecoverTime, 2, false);
    });
}, 5, fireRecoverTime / 2);
function addFire(player) {
    if (!updateWingingPlayer(player))
        return;
    //logger.info("addFire");
...
function updateWingingPlayer(player) {
...
function appendEffect(player, effect, duration, amplifier, showParticles = true) {
