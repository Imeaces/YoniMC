...
execute @s[scores={guxi-opt=1..},rxm=-84] ~ ~ ~ tellraw @s {"rawtext":[{"translate":"中止"}]}
execute @s[scores={guxi-opt=1..},rxm=-84] ~ ~ ~ scoreboard players set @s guxi-opt 0

## 两秒后打开
execute @s[scores={guxi-opt=40..}] ~ ~ ~ scoreboard players set @s guxi-op 11
