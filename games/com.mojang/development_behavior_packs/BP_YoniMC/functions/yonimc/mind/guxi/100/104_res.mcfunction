...
execute unless score @s var_0 matches 0 run scoreboard players operation @s md:res_v0 += @s var_0
execute unless score @s var_0 matches 0 run function yonimc/mind/operation/rotate_y_offset_init

# 限制范围
execute if score @s md:res_v0 matches ..-1 run scoreboard players set @s md:res_v0 0
execute if score @s md:res_v0 matches 12.. run scoreboard players set @s md:res_v0 11
...
#显示当前分数
titleraw @s actionbar {"rawtext": [{"translate": "§l#res: %%s", "with": {"rawtext": [{"score": {"objective": "md:res_v0", "name": "@s" }}]}}, {"translate":"§r%%2§r","with":{"rawtext":[{"selector":"@s[scores={md:timer_076=1..}]"},{"translate":"\n>>#t: %%s","with":{"rawtext":[{"score":{"objective":"md:timer_076","name":"@s"}}]}}]}}, {"translate":"\n§r§7%%s§f|%%s","with":{"rawtext":[{"score":{"objective":"guxi:energy","name":"@s"}},{"score":{"objective":"guxi:energy_pool","name":"@s"}}]}}]}

# 确认设置
execute if score @s var_0 matches 100 run scoreboard players operation @s guxi:ef_res = @s md:res_v0
