...
execute unless score @s var_0 matches 0 run scoreboard players operation @s md:105_v0 += @s var_0
execute unless score @s var_0 matches 0 run function yonimc/mind/operation/rotate_y_offset_init

# 限制范围
execute if score @s md:105_v0 matches ..-1 run scoreboard players set @s md:105_v0 0
execute if score @s md:105_v0 matches 2.. run scoreboard players set @s md:105_v0 1
...
#显示当前分数
titleraw @s actionbar {"rawtext": [{"translate": "§l#res*/: %%2", "with": {"rawtext": [{"selector": "@s[scores={md:105_v0=1}]"},{"text": "#!"},{"text": "#_"}]}}, {"translate":"§r%%2§r","with":{"rawtext":[{"selector":"@s[scores={md:timer_076=1..}]"},{"translate":"\n>>#t: %%s","with":{"rawtext":[{"score":{"objective":"md:timer_076","name":"@s"}}]}}]}}, {"translate":"\n§r§7%%s§f|%%s","with":{"rawtext":[{"score":{"objective":"guxi:energy","name":"@s"}},{"score":{"objective":"guxi:energy_pool","name":"@s"}}]}}]}

# 确认设置
execute if score @s var_0 matches 100 if score @s md:105_v0 matches 0 run scoreboard players set @s guxi:keep_res 0
execute if score @s var_0 matches 100 if score @s md:105_v0 matches 1 run scoreboard players set @s guxi:keep_res 1
